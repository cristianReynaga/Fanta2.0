...
void testApp::setup()
{
    ...
    gui.addToggle("Start interaction", bStartInteraction);
    gui.addSlider("Current Movie", currentMovie, 0, _MAX_MOVIES);
    gui.addSlider("Volumen Pantalla", audioLevel, 0.0f, 1.0f);
    gui.addSlider("Parlantes 1 y 2", audioLevel12, 0, 127);
    gui.addSlider("Parlantes 3 y 4", audioLevel34, 0, 127);
	...
	gui.addToggle("Show Blend", blender.showBlend);
	 for (int i=0; i<PROJECTOR_COUNT-2; i++)//-2
    {
...
//Pide a cámara nuevos frames para mostrar
    ...
    if (currentMovie == ESCENA_CORAZONES && ofGetElapsedTimeMillis()> nextMiniEvent && contourFinder.nBlobs > 3) // > nextMiniEvent && FFTanalyzer.averages[5] > umbralSonido) {
    {
        hearts.addHeart();
...
bool testApp::continueLoop( )
{
    ...
    {
        bLed2=true;
        continueStatus = FFTanalyzer.averages[freqFirstAnalysis] > (umbralSonido+3.0);

    }

    if (currentMovie == ESCENA_FINAL)
    {
